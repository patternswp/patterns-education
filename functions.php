<?php // phpcs:ignore
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

/**
 * Patterns Education functions and definitions
 *
 * @link https://developer.wordpress.org/themes/basics/theme-functions/
 *
 * @package Patterns Education
 */

/**
 * Current theme path.
 * Current theme url.
 * Current theme version.
 * Current theme name.
 * Current theme option name.
 */
define( 'PATTERNS_EDUCATION_PATH', trailingslashit( get_template_directory() ) );
define( 'PATTERNS_EDUCATION_URL', trailingslashit( get_template_directory_uri() ) );
define( 'PATTERNS_EDUCATION_VERSION', '2.0.0' );
define( 'PATTERNS_EDUCATION_THEME_NAME', 'patterns-education' );
define( 'PATTERNS_EDUCATION_OPTION_NAME', 'patterns-education' );

/**
 * The core theme class that is used to define internationalization,
 * admin-specific hooks, and public-facing site hooks.
 */
require PATTERNS_EDUCATION_PATH . 'includes/main.php';

/**
 * Begins execution of the theme.
 *
 * @since    1.0.0
 */
function patterns_education_run() {
	new Patterns_Education();
}
patterns_education_run();
